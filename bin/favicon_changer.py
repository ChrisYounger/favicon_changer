    ...
    def handle_GET(self):
        ...
        try:
            if "query" in self.request and "icon" in self.request['query'] and not self.request['query']['icon'] is None:
                ...
                output += "Setting browser icon to: " + action + "<br />"
                if action == "original":
                    if not os.path.exists(os.path.join(dest_folder, 'favicon.ico.orig')):
                        output += "ERROR: Cannot restore original icon becuase backup file is missing.<br />Restore the file from original installation package.\n"
                    else:
                        shutil.copyfile(os.path.join(dest_folder, 'favicon.ico.orig'), os.path.join(dest_folder, 'favicon.ico'))
                        output += "SUCCESS: Restored original file.<br /><br />Hit CTRL-F5 to clear your browser cache and observe the change.\n"
                else: 
